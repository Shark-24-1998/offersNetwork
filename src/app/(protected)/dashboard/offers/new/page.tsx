...
export default function NewOfferPage() {
  ...
  const [tierSteps, setTierSteps] = useState<TierStep[]>([]);
  const [includedCountries, setIncludedCountries] = useState<string[]>([]);
  const [excludedCountries, setExcludedCountries] = useState<string[]>([]);
  const [countryMode, setCountryMode] = useState<"included" | "excluded">("included");
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/offers", {
        ...
        body: JSON.stringify({
          title: cleanTitle,
          link: cleanLink, // FIXED: was sending cleanTitle instead of cleanLink
          tierWiseSteps: tierSteps,
          includedCountries,
          excludedCountries,
        }),
      });
      ...
      setLoading(false); // FIXED: was setting to true instead of false
    }
  }
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
              ...
              <div className="mb-4 space-y-2">
                {includedCountries.length > 0 && (
                  <div className="flex items-start gap-2">
                    <span className="text-xs font-medium text-green-700 mt-1">Included:</span>
                      ...
                      {includedCountries.map((code) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => setIncludedCountries(prev => prev.filter(c => c !== code))}
                ...
                {excludedCountries.length > 0 && (
                  <div className="flex items-start gap-2">
                    <span className="text-xs font-medium text-red-700 mt-1">Excluded:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {excludedCountries.map((code) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => setExcludedCountries(prev => prev.filter(c => c !== code))}
          ...
          <button
            ...
            disabled={loading}
            className="group relative w-full inline-flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-br from-purple-500 via-indigo-500 to-blue-600 px-6 py-3.5 text-base font-semibold text-white shadow-lg shadow-purple-500/30 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-purple-500/40 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
